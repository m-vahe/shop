              ...
              {leftText?.Text}
                ...
                <div className={"prod-items-bod"}>
                    {products && products.map((e, i) => {
                        return (
                            <div className={' first-prod-items col-lg-3'} key={i} onClick={() => console.log(e)}>
                                <div
                                ...
                                >
                                    <img
                                        src={
                                            e.variants_of_a_products?.find(item => item.main === true)?.images[0]?.url ?
                                                e.variants_of_a_products?.find(item => item.main === true)?.images[0]?.url
                                                : e.images[0]?.url
                                        }
                                    ...
                                    <svg
                                        ...
                                        style={e?.variants_of_a_products?.find(item => item.main === true)?.favorite ? {stroke: '#000000'} : {stroke: '#7b7b7b'}}
                                    >
                                        <path
                                ...
                                {e?.limited_edition ? <span className={'prod-txt-head2'}>Limited edition</span> :
                                    <span style={{opacity: "0"}} className={'prod-txt-head2'}>Limited edition</span>}
                                {e?.brand ? <h3 className={'prod-txt-name'}>{e?.brand}</h3> :
                                    <h3 className={'prod-txt-name'} style={{opacity: "0"}}>jdfbhsbh</h3>
                                }
                                {e?.name ? (
                                ...
                                <h3 className={'prod-txt-price'}>
                                    {formatter.format(e.variants_of_a_products?.find(item => item.main === true)?.price
                                        || 0)}
                                </h3>
